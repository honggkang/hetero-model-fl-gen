...
from utils.localUpdateGenF import LocalUpdate_DCGANF
...
def main():
    ...
    for iter in range(1, args.gen_wu_epochs+1):
        ...
        idxs_users = user_select(args)        
        for idx in idxs_users:                        
            local = LocalUpdate_DCGANF(args, common_net, dataset=dataset_train, idxs=dict_users[idx])
    ...
    for iter in range(1, args.epochs+1):
        ws_local = [[] for _ in range(args.num_models)]
        gen_w_local, dis_w_local, loss_locals, gen_loss_locals, gloss_locals, dloss_locals = [], [], [], [], [], []
        ...
        for idx in idxs_users:
            ...
            if args.only_gen: # update (header; layers after FE) only by synthetic data
                ...
                local = LocalUpdate_header(args, dataset=dataset_train, idxs=dict_users[idx]) # update header by syn+real data
                if args.aid_by_gen:
            ...
            if args.aid_by_gen and not args.freeze_gen:
                local_gen = LocalUpdate_DCGANF(args, common_net, dataset=dataset_train, idxs=dict_users[idx])

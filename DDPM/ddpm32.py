...
class ResidualConvBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, 1, 1), # in ch, out ch, kernel (3,3), stride (1,1), padding (1,1)
...
class DDPM(nn.Module):
    ...
    def sample_image(self, args, sample_num=0):
        ...
        to make the fwd passes efficient, we concat two versions of the dataset,
        one with context_mask=0 and the other context_mask=1
        we then mix the outputs with the guidance scale, w where w>0 means more guidance        
        '''
        with torch.no_grad():
            size = args.img_shape
    ...
    def sample_cond_image(self, args, sample_num=0, condition_index=None):
        ...
        with torch.no_grad():
            ...
            if condition_index  is None:
                ...
                c_i = torch.tensor([condition_index] * sample_num).to(self.device)
            out_c = copy.deepcopy(c_i)
